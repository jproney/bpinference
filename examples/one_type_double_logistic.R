...
simulation_data <- bpsims(mod, simulation_params, z0, times, rep(40,6), c_mat)
...
stan_mod <- rstan::stan_model(file = "one_type_gp_bd.stan")
fit.data <- rstan::sampling(stan_mod, data = dat, control = list(adapt_delta = 0.95), chains = 4, refresh = 1)
