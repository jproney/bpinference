...
e_mat =  rbind(c(2, 0, 0),c(0,2,0),c(0,0,2), c(1,0,1),c(0,1,1),c(0,0,0), c(0,0,0), c(0,0,0))
p_vec = c(1, 2, 3, 3, 3, 1,2,3)
...
generate(mod, priors, "/michorlab/jroney/stanfiles/three_type_benchmark.stan")

ranges = matrix(rep(c(0,1),length(simulation_params)),length(simulation_params),2,byrow = T)
init = uniform_initialize(ranges, 4)


if(file.exists("/michorlab/jroney/compiles/three_type_benchmark.RDS")){
  stan_mod <- readRDS("/michorlab/jroney/compiles/three_type_benchmark.RDS")
} else{
  stan_mod <- stan_model(file = "/michorlab/jroney/stanfiles/three_type_benchmark.stan")
  saveRDS(stan_mod, "/michorlab/jroney/compiles/three_type_benchmark.RDS")
}
