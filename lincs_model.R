functions{
  ...
  real[] moment_ode(real t, real[] state, real[] theta, real[] rdata, int[] idata) {// rdata param not used
    ...
    mt = to_matrix(head(state, ntypes*ntypes), ntypes, ntypes, 0);
    dt = to_matrix(segment(state,ntypes*ntypes+1, ntypes*ntypes*ntypes),ntypes,ntypes*ntypes, 1);//read this in row-major order
...
parameters{
	...
	real<lower=5, upper=15> Theta3;
	real<lower=0, upper=5> Theta4;
...
transformed parameters{
  ...
  for(i in 1:ndep_levels){
    ...
    //BEGIN AUTO-GENERATED CODE FOR FUNCTIONAL DEPENDENCIES
		...
		r_mat[1, p_vec[1]] = ( Theta1 ) + ( ( Theta2 ) / ( ( 1 ) + ( exp(( Theta3 ) * ( ( function_var[i,1] ) - ( Theta4 ) )) ) ) );
		r_mat[2, p_vec[2]] = Theta5;
...
model{
  ...
  //BEGIN AUTO-GENERATED CODE FOR PRIORS
	...
	Theta2 ~ normal(0,0.25);
	Theta3 ~ uniform(5,15);
	Theta4 ~ normal(0,0.25);
