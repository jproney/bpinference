...
new_bp_model = function(E, P, func_deps, nParam, nDep){
  ...
  return(structure(list(E = E, P = P, func_deps = func_deps, nParams = nParam, nDep = nDep), class="bp_model"))
}

validate_bp_model = function(bpm){
  data = unclass(bpm)
  if(!is.vector(data$P) || !all(round(data$P) == data$P) || !all(data$P > 0)){
  ...
  if(!is.matrix(data$E) || !all(round(data$E) == data$E || !all(data$E >= 0))){
...
bp_model = function(E, P, func_deps, nParam, nDep){
  ...
  new_funcs = sapply(1:length(func_deps), function(i){parse(text = func_deps[i])[[1]]})
