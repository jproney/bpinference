...
#' 
#' @export
calculate_moments_td <- function(e_mat,p_vec,r_func, z0_vec,tf){
  if(!is.numeric(e_mat) || !is.numeric(z0_vec) || !is.numeric(tf)){
  ...
  if(!is.vector(p_vec) || !is.vector(z0_vec)){
    stop("p_vec, z0_vec should all be vectors!")
  }
  if(tf < 0){
