...
* A vector of functional dependencies. Sometimes, we may want the rates of various birth events in our model to vary as some function of an environmental variance. To acoomplish this, we specify functional dependencies as strings using a specific syntax. In essence, this syntax expects parameter number `i` to be referenced as `'c[i]'`, and dependent variable number `j` to be referenced as `'x[j]'`. For full detail on this syntax, see the man page for `bp_model`. In this particular case, we have no dependent varianbles. Therefore we only need two parameters, one for birth and one for death. 
...
We now have a `bp_model` object which we can use for simulation and inference. As we did in the one type case, et's pick some parameter values and simulate a lot of data to confirm normality.
...
The data look very normal, which is what we should expect for a large starting population `z0 = 1000`. Are the mean and variance of this distribution similar to what we should expect analytically? We can check using the `calculate_moments` function:
